...
async function resolveEmployeeProfileId(
  ...
  const profileId =
    typeof data?.profileId === 'string' && data.profileId.trim() !== '' ? data.profileId : null;
...
async function lookupEffectiveRate(
  ...
  if (!effectiveQuery.empty) {
    ...
    rate = Number((rateDoc as any).hourlyRate || (rateDoc as any).amount || 0) || 0;
  } else {
    ...
    if (!legacyQuery.empty) {
      ...
      rate = Number((legacyDoc as any).hourlyRate || (legacyDoc as any).amount || 0) || 0;
    }
  }
...
async function computeTimesheetCompensation(
  ...
  if (snapshot && typeof snapshot === 'object') {
    ...
    const employeeId = typeof timesheet?.employeeId === 'string' ? timesheet.employeeId : null;
...
async function userHasFinanceAdmin(context: functions.https.CallableContext): Promise<boolean> {
...
export const payrollScan = functions.region('us-central1').https.onCall(async (data, context) => {
  try {
    // Validate input
    const { periodStart, periodEnd } = data;
    if (!periodStart || !periodEnd) {
  ...
  .https.onCall(async (data, context) => {
    try {
      ...
      timesheetsSnapshot.forEach((doc) => {
        ...
        if (timesheet.employeeProfileId && timesheet.hours && timesheet.hourlyRate) {
