...
export default function TodayTomorrow() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Get bounds for the active tab
        const bounds =
          activeTab === "today" ? getTodayBounds() : getTomorrowBounds();
  ...
  const grouped = useMemo(() => {
    return groupJobsByBusinessDate(jobs);
  }, [jobs]);

  // Get the current date key for the active tab
  const activeDateKey = useMemo(() => {
    const today = getBusinessNow();
    return activeTab === "today"
      ? getBusinessDateKey(today)
      : getBusinessDateKey(new Date(today.getTime() + 24 * 60 * 60 * 1000));
  }, [activeTab]);

  // Get jobs for the active tab
  const activeJobs = useMemo(() => {
    return grouped[activeDateKey] || [];
  ...
  return (
    <div className="space-y-6">
      {/* Tab Navigation */}
      <div className="card-bg rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
              ...
              {formatBusinessDate(activeDateKey, "EEEE, MMMM d")}
            </h3>
          ...
          {activeJobs.length === 0 ? (
            ...
            <div className="space-y-4">
              {activeJobs.map((j) => {
                ...
                const employees = emp.length ? emp : empNamesByJob[j.id] || [];
